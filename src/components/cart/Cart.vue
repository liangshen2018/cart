    ...
    <div class="scroll">
      ...
      <div class="main-container">
        ...
        <table class="group-list theme-border" width="100%" style="table-layout:fixed;">
          ...
          <tr class="bor" v-for="item in cart_ids" :key="item.id">
            <td style="width:5rem" class="group-item-checkbox" :class="{active: item.isSelected}" @click="changeSelect(item.id)"></td>
            <td style="width:5rem" >
            ...
            <td>
              ...
              <ProdOperate v-model="item.count" @addGoods="addGoods(item.id)"></ProdOperate>
            </td>
          </tr>
          </tbody>
        </table>
        <div class="settlement">
          <span class="selectAll selectAll-false" :class="{ 'selectAll-true': isSelectedAll }" @click="toggleAll">全选</span>
          共:
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      return this.cart_ids.reduce((total, item) => {
        if (item.isSelected) {
  ...
  watch: {
    cart_ids: {
      handler(curList) {
        ...
        this.$store.commit('update', { value: cartList })
      },
      deep: true
    }
  },
  methods: {
    addGoods(id) {
      let count = this.cart_ids.find(item => item.id === id).count
    ...
    changeSelect(id) {
      let good = this.cart_ids.find(item => item.id === id)
      good.isSelected = !good.isSelected
    },
    toggleAll() {
      this.cart_ids.forEach(item => {
        item.isSelected = !this.isSelectedAll
      })
    }
  },
  async created() {
    let { ids, newData } = this.$store.getters
    let res = await this.$axios.get(`/api/home?_r=${Math.random()}&
    cart_pids=${ids}&location=121.5721941391567%2C31.21168025925351`)
    console.log(res)
    if (res.status === 200) {
      this.cart_ids = res.data.data.cart_ids.map(item => {
        return {
          ...item,
          count: newData[item.id].count,
          isSelected: newData[item.id].isSelected
        }
      })
    }
  },
